...
Let's say we have made some observations $x$, and we want to make predictions at points $x_\ast$. A GP must be *consistent*, this is an important part of the formal definition of a GP -- a definition I've studiously avoided giving. By consistency we mean that the model makes predictions for $x$ that are unaffected by the future values of $x_\ast$ that are currently unobserved, i.e. any test points. This ties to the property that a GP is akin to regression with an infinite number of basis functions.[^6] The GP is, by definition, an infinite dimensional object but consistency saves us, when looking at the covariance matrix of our GP only the unmarginalized values matter; and the inference that you do with those values will be the same as if you'd used all infinitely many dimensions into account.

Using this property we can write down a joint probability density over the training observations and the test points, $x$ and $x_\ast$.[^7] This joint probability density will be
...
That's a bit of a mouthful, but by sampling from this distribution we can get functions which are conditioned on our observation[^8]
...
[^6]: This comes from Mercer's Theorem.

[^7]: Alright, that was super hand-wavy. Suffice to say it's related to another nice property of Gaussians, the marginalisation property. More details [here](http://www.gaussianprocess.org/gpml/chapters/RW2.pdf).

[^8]: Coding this up this way is actually a really bad idea, you'll hit all sorts of bother with matrix conditioning in that inverse, but it'll do for this example.
